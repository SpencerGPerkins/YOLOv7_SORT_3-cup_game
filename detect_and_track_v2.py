...
from table_mask import getTableMask 
from coordinate_utils import SectionCenters
...
def draw_boxes(img, bbox, identities=None, categories=None, names=None, save_with_object_id=False, path=None,offset=(0, 0)):
    ...
    return img
#................................ Calculate centroids ..............................................
"""Calculate centroids to determine table section"""
def centroid(xyxy):
    x1, y1, x2, y2 = xyxy
    center_x = (x1 + x2) / 2
    center_y = (y1 + y2) / 2
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        #-------------------Table masking and section coords---------------------------------------------#
        if initial_frame == 0:
            img = np.squeeze(img)
            initial_frame += 1 
            points = getTableMask(img)
            section_centers = ROI_sections.get_section_centers(points)
        #-----------------------------------------------------------------------------------------------
        ...
        if device.type != 'cpu' and (old_img_b != img.shape[0] or old_img_h != img.shape[2] or old_img_w != img.shape[3]):
            ...
            old_img_h = img.shape[3]
            old_img_w = img.shape[2]           
        ...
        #-----------------------------ROI filtering for detections------------------------------------------#
        filtered_detections = []
        # Process detections
        for det in pred:  # detections per image
            if len(det) != 0:
                for bbox in det:
                    det_centerX, det_centerY = centroid(bbox[:4].cpu().detach().numpy())
                    if det_centerX < max(points[1][0], points[2][0]): # Define leftside void area
                        continue
                    elif det_centerX > max(points[0][0], points[3][0]): # Define rightside void area
                        continue
                    elif det_centerY < max(points[0][1], points[1][1]): # Define top void area
                        continue
                    elif det_centerY > max(points[2][1], points[3][1]): # Define bottom void area
                        continue
                    else:
                        filtered_detections.append(bbox)
        if len(filtered_detections) != 0:
            pred = [torch.stack(filtered_detections).to(device)]
        else:
            pred = [torch.empty((0, 6), device=device)]
        #------------------------------------------------------------------------------------------------------#

        
        for i, det in enumerate(pred): # For filtered Detections method pred -> filtered_detectiions
            ...
            if len(det):
                ...
                # Run SORT
                tracked_dets, vip_ = sort_tracker.update(dets_to_sort)
                ...
                tracked_dets, vip_ = sort_tracker.update()
            #........................................................
            
            #------------------- Determine Object Location, format to JSON file for LLM ----------------------------------#
            if len(vip_) == 1:
                out_path = "location.json"
                vip_center = centroid(vip_[0][:4])
                min_distance = 100
                section_center_keys = list(section_centers.keys())
                target_section = ""
                # Compare with section Centroids
                for c in range(len(section_center_keys)):
                    VIPsection_distance = euclidean_dist(section_centers[section_center_keys[c]], vip_center)
                    if VIPsection_distance < min_distance:
                        target_section = section_center_keys[c]
                        min_distance = VIPsection_distance
                out_data = {
                    "ball_position": target_section
                }
